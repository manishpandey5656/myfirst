...
class MapGenerator{
    ...
    // this set the value of brick to 0 if it is hit by the ball
...
class GamePlay extends JPanel implements KeyListener,ActionListener {
    ...
    public void paint(Graphics g){
        ...
        if(totalBricks <= 0 ){
            //if all the bricks are destroyed then you win
            play = false;
            ballXdir = 0;
            ballYdir = 0;
        ...
        if(ballposY > 570){
            // if ball goes below the paddle then you loose
            play = false;
            ballXdir = 0;
            ballYdir = 0;
            g.setColor(Color.BLACK);
        ...
        public void actionPerformed(ActionEvent arg0){
            ...
            for(int i =0; i<map.map.length;i++){
                ...
                for(int j = 0; j<map.map[0].length;j++){
                    ...
                    if(map.map[i][j]>0){
                        ...
                        if(ballposY<0){
                            //if ball hits the top wall then it bouncecs back
                            ballYdir = -ballYdir;            
                        }
                        if(ballposX>670){
                            //if ball hits the rigth wall then it bounces back
                            ballXdir = -ballXdir;
                        }
                       } 
                        repaint();
            ...
            public void keyPressed(KeyEvent arg0){
                ...
                if(arg0.getKeyCode()==KeyEvent.VK_LEFT){
                    //if left arrow is pressed then paddle moves left
                    if(playerX< 10){
                        playerX = 10;
                    }
                    else{
                ...
                if(arg0.getKeyCode()==KeyEvent.VK_ENTER){
                    //if enter is pressed then game restarts
                    if(!play){
                        play=true;
                        ballposX = 120;
                        ballposY = 350;
